...
void loop()  // Start of loop:
{
  ...
  { Culoare(Led, 0, 255, 0);//VERDE
    ...
    if (rpm > RPM) //daca cadenta e prea mare
    {
      viteza++;
      delay(125);
    }
    else if (rpm < RPM) //daca cadenta e prea mica
    { viteza--;
      delay(125);
    }
  }
  servo.write(v[viteza]); Serial.println(v[viteza]);
  delay(25);
}  // End of loop.

void Culoare(int r, int g, int b)
{
  analogWrite(Led[1], r);//ledul rosu
  analogWrite(Led[2], g);//ledul verde
  analogWrite(Led[3], b);//ledul albastru
}

double batteryVoltage()
{
  int volt = analogRead(A0);// read the input
  double voltage = map(volt, 0, 1023, 0, 2500) + 20; // map 0-1023 to 0-2500 and add correction offset
