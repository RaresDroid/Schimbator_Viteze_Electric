...
void loop()  // Start of loop:
{
  ...
  else if (mod == 2)//AUTO
  {
    Culoare(190, 0);//Green
    if (digitalRead(Buton1) == HIGH)
    { if (cadenta < 50)cadenta += 20, mRpm = cadenta, RPM = cadenta;
      delay(125);
    }
    else if (digitalRead(Buton2) == HIGH)
    { if (cadenta > 30)cadenta -= 20, mRpm = cadenta, RPM = cadenta;
      delay(125);
    }
  
    if (timeOut >= 60)
    { if (mRpm - cadenta > 7  && mRpm >= cadenta) //daca cadenta e prea mare
      {
        down = 0;
        if (viteza < 8)
          viteza++, mRpm = cadenta,timeOut=0;
        delay(125);
      }
      else if (cadenta - mRpm > 7 && cadenta >= mRpm) //daca cadenta e prea mica
      {
        if (viteza > 1)
        { down = f[viteza];
          viteza--;
  ...
  servo.write(v[viteza] - down);
  Serial.print("cadenta= ");
  Serial.print(cadenta);
  Serial.print("\tposition= ");
  Serial.print(v[viteza] - down);
  Serial.print("\tdifS= ");
  Serial.print(mRpm - cadenta);
  Serial.print("\tdifJ= ");
  Serial.println(abs(cadenta - mRpm));
  delay(25);
}  // End of loop.

void Culoare(int g, int b)
{
  
  analogWrite(A4, g);//green led
  analogWrite(A5, b);//blue led
}

double batteryVoltage()
{
  int volt = analogRead(A0);// read the input
  double voltage = map(volt, 0, 1023, 0, 2500) -67; // map 0-1023 to 0-2500 and add correction offset
